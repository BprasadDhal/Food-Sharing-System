...
<!--             
   ...
   <div class="table-container">
       ...
       <tbody>
        ...
        $query="select * from receive_food where email='$email'";
        $result=mysqli_query($connection, $query);
        if($result==true){
            while($row=mysqli_fetch_assoc($result)){
                echo "<tr><td>".$row['food']."</td><td>".$row['type']."</td><td>".$row['category']."</td><td>".$row['date']."</td><td><a href='receiver_track_details.php?track_id=$row[rid]'>Track Order</a></td></tr>";
