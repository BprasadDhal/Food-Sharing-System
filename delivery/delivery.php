...
LEFT JOIN login ad ON fd.assigned_to = ad.id where assigned_to IS NOT NULL and   delivery_by IS NULL and fd.location='$city';
";
$sql_receive = "SELECT rf.rid AS rid,rf.location as cure, rf.name,rf.phoneno,rf.date,rf.delivery_by, rf.adress as From_adress, 
ad.name AS delivery_person_name, ad.adress AS To_address
FROM receive_food rf
LEFT JOIN login ad ON rf.assigned_to = ad.id where assigned_to IS NOT NULL and   delivery_by IS NULL and rf.location='$city';
";
...
if (isset($_POST['food_receive']) && isset($_POST['delivery_person_id_receive'])) {
    ...
    $sql_receive = "SELECT * FROM receive_food WHERE rid = $order_id AND delivery_by IS NOT NULL";
    $result_receive = mysqli_query($connection, $sql_receive);
    ...
    $sql_receive = "UPDATE receive_food SET delivery_by = $delivery_person_id WHERE rid = $order_id";
    // $result = mysqli_query($conn, $sql);
    $result=mysqli_query($connection, $sql_receive);
...
?>
       ...
       <tbody>
        ...
        <?php foreach ($data_receive as $row_receive) { ?>
        <?php    echo "<tr><td data-label=\"name\">".$row_receive['name']."</td><td data-label=\"phoneno\">".$row_receive['phoneno']."</td><td data-label=\"date\">".$row_receive['date']."</td><td data-label=\"Pickup Address\">".$row_receive['To_address']."</td><td data-label=\"Delivery Address\">".$row_receive['From_adress']."</td>";
?>
        
            <!-- <td><?= $row_receive['rid'] ?></td>
            <td><?= $row_receive['name'] ?></td>
            <td><?= $row_receive['address'] ?></td> -->
            <td data-label="Action" style="margin:auto">
                <?php if ($row_receive['delivery_by'] == null) { ?>
                    <form method="post" action=" ">
                        <input type="hidden" name="order_id" value="<?= $row_receive['rid'] ?>">
                        <input type="hidden" name="delivery_person_id_receive" value="<?= $id ?>">
                        <button type="submit" name="food_receive">Take order</button>
                    </form>
                <?php } else if ($row_receive['delivery_by'] == $id) { ?>
                    Order assigned to you
                <?php } else { ?>
                    Order assigned to another delivery person
                <?php } ?>
